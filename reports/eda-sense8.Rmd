...
```{r}
friends <- episodes %>% filter (series_name == 'Friends') %>%
    group_by(season) %>% 
    summarise(correlacao_linear_ep = cor(season_ep, user_rating, 
                                      method = "pearson"), 
              correlacao_kendall_ep = cor(season_ep, user_rating, 
                                       method = "kendall"),
              correlacao_kendall_season = cor(season_ep, user_rating, 
                                       method = "kendall"),
              ) 
friends %>% summarise(correlacao_inicio_fim = cor(season, user_rating))
  ggplot(aes(x = season, y = correlacao_kendall)) +
  geom_point()
```
